...
var tableroAux = crearTableroAux();
            ...
            if (tablero[i][j] == 0) { //detecta lugares vacios
                ...
                if (minimaCant <= CeldasVacias (llenarCeldaOcup(tableroAux))) { 
                    if (minimaCant < CeldasVacias(llenarCeldaOcup(tableroAux))){
                        ...
                        vJ.push(j);
                        minimaCant = CeldasVacias(llenarCeldaOcup(tableroAux));
                        cantCeldasVacias = 0;
                    }
                }
            }    
...
function llenarCeldaOcup(tablero){ // Para llenar en que lugares no se puede colocar reinas
    var acum = 0;
        for (let i = 0; i < 8; i++) {
            for (let j = 0; j < 8; j++) {
...
function CeldasVacias(tablero){
    ...
    console.log(' CeldasVacias el valor de acumulador es',acum);
    return acum;
}
